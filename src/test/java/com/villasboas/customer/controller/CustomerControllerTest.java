...
class CustomerControllerTest {
	...
	void testWhenPutThenUpdateAllFields() throws Exception {
		...
		mockMvc.perform(configureHeaders(put(putUrl)).content(content))
				.andDo(print())
				.andExpect(status().isNoContent());
		...
		mockMvc.perform(configureHeaders(get(getUrl)))
				.andDo(print())
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.id", is(id.toString()))).andExpect(jsonPath("$.name", is("Alice")))
				.andExpect(jsonPath("$.cpf", is("06539014023"))).andExpect(jsonPath("$.birthDate", is("2019-12-12")));
	...
	void testWhenPatchThenUpdateOnlyInformedProperties() throws Exception {
		...
		mockMvc.perform(configureHeaders(patch(url)).content(content))
	...
	@Test
	void testWhenPostWithInvalidFieldValuesThenValidateAndReturnAn400() throws Exception {

		final String customerDtoAsString = createCustomerDtoWithEmptyFields();
		final String url = "/api/customers";

		final ResultActions resultActions = mockMvc.perform(configureHeaders(post(url)).content(customerDtoAsString));

		final ArrayList<String> expectedOrderedResult = Lists.newArrayList("id cannot be null", "name cannot be null", "cpf cannot be null",
				"birthDate cannot be null");
		expectedOrderedResult.sort((s1,s2) -> s1.compareTo(s2));
		
		resultActions
				.andDo(print())
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.errorMessage", is("Error when validate data.")))
				.andExpect(jsonPath("$.errorDetails.length()", is(4)))
				.andExpect(jsonPath("$.errorDetails[0]", is(expectedOrderedResult.get(0))))
				.andExpect(jsonPath("$.errorDetails[1]", is(expectedOrderedResult.get(1))))
				.andExpect(jsonPath("$.errorDetails[2]", is(expectedOrderedResult.get(2))))
				.andExpect(jsonPath("$.errorDetails[3]", is(expectedOrderedResult.get(3))));

	}

	private String createCustomerDtoWithEmptyFields() throws JsonProcessingException {
